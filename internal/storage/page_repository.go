...
func (r *PageRepository) Create(ctx context.Context, p *models.Page) error {
	...
	`
	err := r.db.QueryRow(ctx, query, p.CrawlID, p.URL, p.StatusCode, p.ContentType, p.ContentLength, p.ResponseTimeMs, p.Title, p.HTML, p.Error).Scan(&p.ID, &p.CreatedAt)
	if err != nil {
...
func (r *PageRepository) BulkInsert(ctx context.Context, pages []*models.Page) (int64, error) {
	...
	for i, p := range pages {
		rows[i] = []interface{}{
			p.CrawlID, p.URL, p.StatusCode, p.ContentType, p.ContentLength, p.ResponseTimeMs, p.Title, p.HTML, p.Error,
		}
	}
	...
	if err != nil {
		return 0, fmt.Errorf("bulk insrting %d pages: %w", len(pages), err)
	}
